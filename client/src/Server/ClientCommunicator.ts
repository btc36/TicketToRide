...
export class ClientCommunicator {
  ...
  public executeCommands(commands: ClientCommandObjects[]){
    for (var i = 0; i < commands.length; i++){
      ...
      // else if (commands[i]._methodName == "endTurn") {
      //   this.inGameClientFacade.endTurn(commands[i]._paramValues[3], commands[i]._paramValues[5]);
      // }
      // else if (commands[i]._methodName == "endGame") {
      //   this.inGameClientFacade.endGame(commands[i]._paramValues[3], commands[i]._paramValues[5]);
      // }
    }
  }
}
